...
def obtener_gestiones_pendientes():
    ...
    try:
        ...
        return {"con_codigo": [], "sin_codigo": [], "fecha_actual": datetime.now().strftime("%Y-%m-%d")}
...
def actualizar_proxima_gestion(id_gestion, id_llamada, tiene_codigo):
    ...
    try:
        ...
        proxima_fecha = calcular_proxima_fecha(fecha_actual, int(id_llamada), tiene_codigo)
        
        if proxima_fecha:
            # Actualizar la fecha de próxima gestión
            query_update = "UPDATE gestiones SET fechaProximaGestion = %s WHERE id_gestion = %s"
...
def registrar_nueva_gestion(celular, id_llamada, id_estado, id_tipificacion, canal, comentario, tiene_codigo):
    """
